...
def process_frame_hsv(frame, dico):
	...
	#	    color=(255, 255, 255), thickness=-1)
            ...
            #    brown_nb_left += 1
            # if coord_is_in_center(coord[0], dico["width"]):
            # if dico["COMPUTER_USED"] or dico["VIDEO_FEEDBACK"]:
            #    output = cv2.circle(output, coord[0], radius=0,
            #                        color=(255, 255, 255), thickness=-1)
            #  brown_nb_center += 1
            ...
            #                            color=(255, 255, 255), thickness=-1)
            #    brown_nb_right += 1
            brown_center_percentage = (len(brown_coords)) / \
                (dico["width"]*abs(dico["brown_top_band"]-dico["brown_bot_band"]))
            print(
                f"switch_ready:{dico['switch_ready']}\t \
                current_color:{dico['color_string'][dico['current_color']]}\t \
                l:{brown_nb_left}\t c:{brown_nb_center}\t r:{brown_nb_right}\t p:{round(brown_center_percentage,4)}")
        else:
            brown_center_percentage = 0
            brown_coords = [0]
        if not dico["switch_ready"] and brown_center_percentage < 0.1:
            dico["switch_ready"] = True
        elif dico["switch_ready"] and 0.5 <= brown_center_percentage and brown_center_percentage <= 1:
            next_color(dico)
